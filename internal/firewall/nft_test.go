...
func TestEnforceModeRedirectsTCPAndDNSAndBlocksOtherUDP(t *testing.T) {
	...
	mustContainCommand(t, plan.Commands, "nft add rule inet box_deadbeef input iifname vethhdeadbeef ip saddr 100.96.0.0/30 meta l4proto tcp tcp dport 18080 accept")
	mustContainCommand(t, plan.Commands, "nft add rule inet box_deadbeef input meta l4proto tcp tcp dport 18080 drop")
	mustContainCommand(t, plan.Commands, "nft add rule inet box_deadbeef input iifname vethhdeadbeef ip saddr 100.96.0.0/30 meta l4proto tcp tcp dport 19001 accept")
	mustContainCommand(t, plan.Commands, "nft add rule inet box_deadbeef input meta l4proto tcp tcp dport 19001 drop")
...
func TestMonitorModeRedirectsTCPAndDNSAndBlocksOtherUDP(t *testing.T) {
	...
	mustContainCommand(t, plan.Commands, "nft add rule inet box_deadbeef input iifname vethhdeadbeef ip saddr 100.96.0.0/30 meta l4proto tcp tcp dport 19001 accept")
	mustContainCommand(t, plan.Commands, "nft add rule inet box_deadbeef input meta l4proto tcp tcp dport 19001 drop")
	mustContainCommand(t, plan.Commands, "nft add rule inet box_deadbeef input iifname vethhdeadbeef ip saddr 100.96.0.0/30 meta l4proto udp udp dport 53 accept")
	mustContainCommand(t, plan.Commands, "nft add rule inet box_deadbeef input meta l4proto udp udp dport 53 drop")
	mustContainCommand(t, plan.Commands, "nft add rule inet box_deadbeef forward iifname vethhdeadbeef ip saddr 100.96.0.0/30 meta l4proto udp drop")
	mustContainCommand(t, plan.Commands, "nft add rule inet box_deadbeef forward iifname vethhdeadbeef ip saddr 100.96.0.0/30 meta l4proto icmp accept")
	mustContainCommand(t, plan.Commands, "nft add rule inet box_deadbeef forward iifname vethhdeadbeef ip saddr 100.96.0.0/30 drop")
}
